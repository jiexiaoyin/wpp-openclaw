...
//
// 编码器/解码器路径:
//   - 编码: /usr/local/silk_decoder/silk/encoder (MP3/PCM → silk for outbound 语音发送)
//   - 解码: /usr/local/silk_decoder/silk/decoder (silk → PCM for STT)
//
...
//   生产不设 env → 默认 /usr/local/silk_decoder/ (行为不变)
const DEFAULT_SILK_ENCODER_PATH = process.env.WPP_SILK_ENCODER_PATH || "/usr/local/silk_decoder/silk/encoder";
const DEFAULT_SILK_DECODER_PATH = process.env.WPP_SILK_DECODER_PATH || "/usr/local/silk_decoder/silk/decoder";
/**
 * 解码 silk → PCM
