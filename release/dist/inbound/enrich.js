...
import { tryIndependentTrigger, defaultHeartflowConfig, } from "./heartflow-trigger.js";
/**
 ...
 */
export async function enrichAndSaveMessage(msg, cfg) {
    try {
        await saveMessage({
 ...
 */
async function tryHeartflowAfterEnrich(msg, cfg) {
    // 仅群消息触发
    if (msg.peerKind !== "group" || !msg.chatroomId)
        return;
    // 心流关闭或 independentTrigger 未开 → 跳过
    if (!cfg.enabled || !cfg.independentTrigger)
        return;
    ...
    try {
        const result = await tryIndependentTrigger({
 ...
 */
export async function enrichBatch(batch, cfg) {
