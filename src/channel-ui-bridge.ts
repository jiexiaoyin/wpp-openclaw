...
export interface CoreFieldSpec {
  ...
  enumValues?: string[];
  help?: string;
  /** schema readOnly (如 selfWxid 身份): 页面只读展示, block→file 永不写; 反镜像 file→block 照常 */
  readOnly?: boolean;
}
...
const CORE_FIELDS_FALLBACK: CoreFieldSpec[] = [
...
function schemaNodeToKind(node: Record<string, unknown>): { kind: CoreFieldKind; enumValues?: string[] } {
  ...
  if (t === "array") return { kind: "stringArray" };
  if (Array.isArray(node.enum) && (node.enum as unknown[]).every((x) => typeof x === "string")) {
    return { kind: "enum", enumValues: node.enum as string[] };
  }
  return { kind: "string" };
}

function deriveCoreFields(props: Record<string, unknown>, prefix = "", out: CoreFieldSpec[] = []): CoreFieldSpec[] {
  for (const [key, raw] of Object.entries(props)) {
    if (SECRET_KEY_RE.test(key)) continue; // 明文 secret 永不进写回集
...
export const CHANNEL_UI_CORE_FIELDS: CoreFieldSpec[] = loadChannelUiCoreFields();
...
export function openClawJsonPath(): string {
  return join(resolveOpenClawStateDir(), "openclaw.json");
}

interface OpenClawRootCfg {
  channels?: { wechatpadpro?: unknown };
}

/** 读 openclaw.json (损坏/缺失 → null, 不阻塞插件) */
export function readOpenClawJson(): OpenClawRootCfg | null {
  try {
    return JSON.parse(readFileSync(openClawJsonPath(), "utf8")) as OpenClawRootCfg;
  } catch {
    return null;
  }
}
...
// ---------------------------------------------------------------------------
function normalizeSpecValue(spec: CoreFieldSpec, raw: unknown): { ok: boolean; value?: unknown; reason?: string } {
  switch (spec.kind) {
    case "boolean": {
    ...
    case "string": {
      if (typeof raw !== "string") return { ok: false, reason: `expected string, got ${JSON.stringify(raw)}` };
      const v = raw.trim();
    ...
    case "enum": {
      if (typeof raw !== "string") return { ok: false, reason: `expected string enum, got ${JSON.stringify(raw)}` };
      const v = raw.trim();
    ...
    case "number": {
      ...
      if (!Number.isFinite(n)) return { ok: false, reason: `expected number, got ${JSON.stringify(raw)}` };
...
// 深层读写 (仅叶子, 保留兄弟键)
// ---------------------------------------------------------------------------
function readDeepPath(obj: Record<string, unknown>, dot: string): unknown {
...
function setDeepPathMut(obj: Record<string, unknown>, dot: string, value: unknown): void {
...
// ---------------------------------------------------------------------------
export function extractChannelAccountsBlock(block: unknown): Map<string, Record<string, unknown>> {
  const out = new Map<string, Record<string, unknown>>();
  if (!isPlainRecord(block)) return out;

  // 平铺字段 → default 账号 (仅当块内含核心键或任意明文标量; 排除纯容器 accounts)
  const topScalar: Record<string, unknown> = {};
  ...
  const accounts = isPlainRecord(block.accounts) ? (block.accounts as Record<string, unknown>) : {};

  // 收集有效账号 id (同时平铺默认兜底)
  const ids = new Set<string>();
  for (const id of Object.keys(accounts)) if (isValidAccountId(id)) ids.add(id);
  if (hasTopField) ids.add(DEFAULT_ACCOUNT_ID);

  if (hasTopField) {
    const merged: Record<string, unknown> = { ...topScalar };
    const nested = accounts[DEFAULT_ACCOUNT_ID];
    if (isPlainRecord(nested)) Object.assign(merged, nested);
    out.set(DEFAULT_ACCOUNT_ID, merged);
  }
  for (const id of ids) {
    if (id === DEFAULT_ACCOUNT_ID) continue;
    const nested = accounts[id];
    if (isPlainRecord(nested)) out.set(id, { ...nested });
  }
  return out;
}

// ---------------------------------------------------------------------------
// 核心字段 merge 写回 accounts/<id>.json (一次读改写, 原子, 保留全部未暴露字段)
// ---------------------------------------------------------------------------
...
export async function applyChannelBlockToAccount(
  accountId: string,
  valueBlock: Record<string, unknown>,
): Promise<AccountSyncResult> {
  const res: AccountSyncResult = { ok: false, accountId, created: false, changedFields: [], skipped: [] };
  ...
  for (const spec of CHANNEL_UI_CORE_FIELDS) {
    if (spec.readOnly) continue; // 只读身份字段 (selfWxid): 页面展示用, 永不 block→file
    const rawValue = readDeepPath(valueBlock, spec.key);
    if (rawValue === undefined) continue; // 键缺席 → 不动 (不清空)
    ...
    if (leafEquals(readDeepPath(raw, spec.path), norm.value)) continue; // 值未变
  ...
  if (res.changedFields.length === 0) {
    res.ok = true;
    if (created) res.reason = "no core fields to bootstrap"; // 空块新建 → 不落空文件
    return res;
  }
  ...
  try {
    ...
    res.reason = `write ${accountId}.json failed: ${(e as Error).message}`;
    return res;
  }
  ...
  res.created = created;
  log.info(
    `${CHANNEL_UI_LOG_TAG} account=${accountId}${created ? " (created from channel config)" : ""} synced fields: ${res.changedFields.join(", ")}`,
  );
  return res;
}
...
// ---------------------------------------------------------------------------
export async function syncAccountsFromChannelConfig(
  opts: { onAccountEnabled?: (accountId: string) => void | Promise<void> } = {},
): Promise<AccountSyncResult[]> {
  const root = readOpenClawJson();
  const block = root?.channels?.wechatpadpro;
  const results: AccountSyncResult[] = [];
  if (!isPlainRecord(block)) return results; // 无 channels.wechatpadpro 块 → 无可同步 (不误写)
...
async function readAccountEnabled(accountId: string): Promise<boolean | undefined> {
  if (!isValidAccountId(accountId)) return undefined;
  try {
    const raw = JSON.parse(await readFile(join(await findPluginRoot(), "accounts", `${accountId}.json`), "utf8")) as { enabled?: unknown };
...
function stringifyConfigFile(cfg: Record<string, unknown>): string {
  return JSON.stringify(cfg, null, 2) + "\n";
}

/** 读账号文件原始核心字段 (只取白名单; 不含任何注入的明文凭证) */
async function readCoreFieldsFromAccountFile(accountId: string): Promise<Record<string, unknown>> {
  if (!isValidAccountId(accountId)) return {};
  try {
    const raw = JSON.parse(await readFile(join(await findPluginRoot(), "accounts", `${accountId}.json`), "utf8")) as Record<string, unknown>;
    const out: Record<string, unknown> = {};
    for (const spec of CHANNEL_UI_CORE_FIELDS) {
      const v = readDeepPath(raw, spec.path);
      if (v !== undefined) setDeepPathMut(out, spec.key, v);
    }
    return out;
  } catch {
 ...
 */
export async function publishAccountCoreFieldsToChannelConfig(accountId: string): Promise<{ ok: boolean; changedFields: string[]; reason?: string }> {
  const res = { ok: false as boolean, changedFields: [] as string[], reason: "" as string | undefined };
  if (!mirrorEnabled()) {
  ...
  try {
    ...
    const cfg = JSON.parse(exists) as Record<string, unknown>;
    const channelBlock = cfg.channels && typeof cfg.channels === "object" ? (cfg.channels as Record<string, unknown>).wechatpadpro : undefined;
    ...
    if (accountId === DEFAULT_ACCOUNT_ID) {
      if (hasAccounts) {
        if (typeof (block.accounts as Record<string, unknown>)[DEFAULT_ACCOUNT_ID] !== "object") (block.accounts as Record<string, unknown>)[DEFAULT_ACCOUNT_ID] = {};
        target = (block.accounts as Record<string, unknown>)[DEFAULT_ACCOUNT_ID] as Record<string, unknown>;
      } else {
        target = block;
      }
    } else {
      ...
      if (typeof (block.accounts as Record<string, unknown>)[accountId] !== "object") (block.accounts as Record<string, unknown>)[accountId] = {};
      target = (block.accounts as Record<string, unknown>)[accountId] as Record<string, unknown>;
    }
    ...
    (cfg.channels as Record<string, unknown>).wechatpadpro = block;
    ...
    res.reason = `publish failed: ${(e as Error).message}`;
...
// ---------------------------------------------------------------------------
let bridgeWatchTimer: NodeJS.Timeout | null = null;
let bridgeWatchDebounceMs = 400;
let bridgeWatchActive = false;
let bridgeSyncing = false;

/** 测试用: 调整防抖间隔 */
export function setBridgeWatchDebounceMs(ms: number): void {
  bridgeWatchDebounceMs = ms;
}

export function isWatchingOpenClawChannelConfig(): boolean {
  return bridgeWatchActive;
}

export async function watchOpenClawChannelConfig(
  opts: { onAccountEnabled?: (accountId: string) => void | Promise<void> } = {},
): Promise<() => void> {
  ...
  const syncOnce = async (reason: string): Promise<void> => {
    ...
    try {
      ...
      log.warn(`${CHANNEL_UI_LOG_TAG} sync failed (${reason}): ${(e as Error).message}`);
    } finally {
      bridgeSyncing = false;
    }
  };
  ...
  const handleChange = (eventType: string, filename: string | null): void => {
    if (filename && filename !== "openclaw.json") return;
    if (bridgeWatchTimer) clearTimeout(bridgeWatchTimer);
...
export function channelUiStatus(): { watching: boolean; coreFields: number; blockPresent: boolean } {
  const root = readOpenClawJson();
  const block = root?.channels?.wechatpadpro;
  return {
