...
export type CommandCheckResult =
  ...
  | {
      isCommand: true;
      allowed: true;
      /** 命令名 (e.g. "reset" / "status" / "help") */
      name: string;
      /** 命令剩余参数 (e.g. "/reset --all" → name="reset", args="--all") */
      args: string;
    }
  | {
      isCommand: true;
...
export interface CommandAllowlistConfig {
  /**
   * 允许的命令名白名单 (e.g. ["reset", "status", "help", "ping"])
   * 空数组 = 禁用命令机制 (所有 /xxx 都被 AI 当普通消息处理, 默认行为)
   */
  allowlist: string[];
  /** 命令前缀 (默认 "/") */
  prefix?: string;
