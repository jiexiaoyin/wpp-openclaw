...
export function makeWppSayHello(ctx: WppAccountCtx) {
  ...
  return {
     ...
     */
    modelv1: (url: string, verifyContent = "") =>
      dispatch("/SayHello/Modelv1", verifyContent ? { url, verifyContent } : { url }),
     ...
     */
    modelv2: (toUserName: string, content = "", scene = 15) =>
      dispatch("/SayHello/Modelv2", {
        toUserName,
        content,
        scene,
        fromScene: 0,
     ...
     */
    modelv3: (scene: number, v3: string, v4 = "", verifyContent = "") =>
      dispatch("/SayHello/Modelv3", {
        scene,
        v3,
        ...(v4 ? { v4 } : {}),
