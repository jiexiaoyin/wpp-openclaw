...
export function makeWppFriendCircle(ctx: WppAccountCtx) {
  ...
  return {
    ...
    pushComment: (id: string, forwardAddr: string) =>
      dispatch("/FriendCircle/PushCommnet", { id, forwardAddr }),
     ...
     */
    setBackgroundImage: (url: string, thumbUrl = "") => {
      assertFriendCirclePublishAllowed(ctx.accountId); // v1.3.63 P1: 补 guard (原漏网)
     ...
     */
    batchDownload: (toWxid: string, opt?: { since?: string; until?: string; maxPages?: number; includeOriginal?: boolean }) =>
      dispatch("/FriendCircle/BatchDownload", {
        towxid: toWxid,
    ...
    batchDownloadStatus: (taskId: string) =>
      getWppJson(ctx.baseUrl, `/FriendCircle/BatchDownloadStatus?task_id=${encodeURIComponent(taskId)}`, opts),
     ...
     */
    batchDownloadFile: (taskId: string, name = "") =>
      getWppBinary(
        ctx.baseUrl,
        `/FriendCircle/BatchDownloadFile?task_id=${encodeURIComponent(taskId)}${name ? `&name=${encodeURIComponent(name)}` : ""}`,
        opts,
      ),
     ...
     */
    autoForward: (enabled: boolean, opt?: {
      sourceWxid?: string;
      delaySeconds?: number;
      locationMode?: 0 | 1;
      blackList?: string | string[];
      contentOwnerAuthorized?: boolean;
    }) => {
      if (enabled) assertFriendCirclePublishAllowed(ctx.accountId); // 开启跟发 = 开启自动发布
    ...
    autoForwardStatus: () =>
      getWppJson(ctx.baseUrl, "/FriendCircle/AutoForwardStatus", opts),
  };
}
