...
export const OFFICIAL_ACCOUNTS_META: ToolMeta = {
  ...
  followOfficialAccount: [
    ...
    (biz: string, operation: "follow" | "unfollow") => getOfficialAccountsApi().follow(biz, operation),
  ],
  /** /OfficialAccounts/Quit */
  quitOfficialAccount: [
    "取消关注公众号.",
    Type.Object({ biz: Type.String() }),
    (biz: string) => getOfficialAccountsApi().quit(biz),
  ],
  ...
  likeOfficialAccountArticle: [
    ...
    Type.Object({ url: Type.String() }),
    (url: string) => getOfficialAccountsApi().getAppMsgExtLike(url),
  ],
  /**
  ...
  preVerifyOfficialAccountJsapi: [
    ...
    (appId: string) => getOfficialAccountsApi().jsapiPreVerify(appId),
  ],
  /**
  ...
  getOfficialAccountA8Key: [
    "获取公众号文章 key 和 uin.",
    Type.Object({ url: Type.String() }),
    (url: string) => getOfficialAccountsApi().mpGetA8Key(url),
  ],
  /**
  ...
  authorizeOfficialAccount: [
    ...
    (url: string, appid: string) => getOfficialAccountsApi().oauthAuthorize(url, appid),
  ],
  /**
  ...
  requestOfficialAccountQrAuthorize: [
    "公众号二维码授权请求 (获取授权二维码).",
    Type.Object({ url: Type.String() }),
    (url: string) => getOfficialAccountsApi().qrConnectAuthorize(url),
  ],
  /**
