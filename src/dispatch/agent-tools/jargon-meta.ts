 ...
 * - 中文用子串包含
 */
export function containsJargon(term: string, text: string): boolean {
...
/** 查群黑话 (默认当前会话所在群) */
async function queryJargon(groupId: string, text?: string): Promise<string> {
...
/** 列出群内已挖掘黑话 */
async function listJargon(groupId: string): Promise<string> {
  const accountId = getCurrentAccountId() ?? "default";
  let terms = [];
  try {
    terms = await getJargonTerms(accountId, groupId, 100);
  ...
  if (terms.length === 0) return `群 ${groupId} 暂无已记录的黑话。`;
  const lines = terms
    .filter((t) => t.is_jargon !== 0)
    .map((t) => `- ${t.term}: ${t.meaning || "（暂无含义）"} (频率${t.frequency ?? 1})`);
...
export const JARGON_META: ToolMeta = {
  /** 查询消息中是否含群黑话, 并解释含义 */
  query_jargon: [
    ...
    (groupId: string, text?: string) => queryJargon(groupId, text),
  ],
  /** 列出群内已挖掘的黑话 */
  list_jargon: [
    ...
    (groupId: string) => listJargon(groupId),
  ],
};
