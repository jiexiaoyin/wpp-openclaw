...
export const MSG_META: ToolMeta = {
  ...
  sendMiniProgram: [
    ...
    Type.Object({
      ...
      thumbUrl: Type.Optional(
        Type.String({
          description:
            "缩略图: 图片URL, 或转发原卡片时的凭据令牌 (入站注记 [小程序封面凭据] 那串 xcxthumb:…) —— 令牌会原样透传, 卡片图与原卡片一致 (v1.6.4)",
        }),
      ),
    ...
    (
      ...
      pagePath?: string,
      xcxUsername?: string,
    ) => getMsgApi().sendXCX(toWxid, xcxTitle, xcxDesc, xcxUrl, xcxAppId, thumbUrl, pagePath, xcxUsername),
  ],
  /** /Msg/ShareCard */
  sendContactCard: [
    "分享联系人名片.",
  ...
  sendMessage: [
    ...
    Type.Object({
      ...
      thumbUrl: Type.Optional(
        Type.String({
          description:
            "image/video/link: 缩略图URL; miniprogram: 转发原卡片时填 [小程序封面凭据] 的 xcxthumb:… 令牌 (图与原卡片一致), 或填图片URL走上传",
        }),
      ),
    ...
    async (toWxid: string, type: string, content: string, fileName?: string, title?: string, desc?: string,
      ...
      pagePath?: string, username?: string) => {
