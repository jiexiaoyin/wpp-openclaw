...
export const QW_CONTACT_META: ToolMeta = {
  qwContactSearch: ["搜索企业微信联系人 (username=企业联系人).", Type.Object({ username: Type.String() }), (username: string) => getQwc().searchQWContact(username)],
  qwContactApply: [
    "企业微信申请加好友 (username=联系人标识, v1=验证凭据).",
    Type.Object({ username: Type.String(), v1: Type.String(), context: Type.Optional(Type.String()) }),
    (username: string, v1: string, context?: string) => getQwc().qwApplyAddContact(username, v1, context ?? ""),
  ],
  qwContactAdd: [
    "企业微信主动加好友 (username=搜索结果联系人, v1=验证凭据).",
    Type.Object({ username: Type.String(), v1: Type.String() }),
    (username: string, v1: string) => getQwc().qwAddContact(username, v1),
  ],
};
