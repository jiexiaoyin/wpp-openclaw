...
export const USER_META: ToolMeta = {
  ...
  changePassword: [
    ...
    (newPwd: string, ticket: string) => getUserApi().setPasswd(newPwd, ticket),
  ],
  /** /User/VerifyPasswd */
  verifyPassword: [
    "验证当前密码 (用于敏感操作前).",
  ...
  reportMotion: [
    ...
    (steps: number, deviceId?: string, deviceType?: string) =>
      getUserApi().reportMotion(steps, deviceId ?? "", deviceType ?? "ipad"),
  ],
  /** /User/BindingMobile */
  bindMobile: [
    "换绑手机号.",
