...
function isSendOk(r: { Code: number; Data?: unknown }): boolean {
  ...
  const d = r.Data as { BaseResponse?: { ret?: number }; List?: Array<{ Ret?: number }> } | undefined;
  // per-item Ret: 任一条非 0 → 实际拒收 (逐条失败)
  if (Array.isArray(d?.List) && d.List.length > 0) {
    for (const item of d.List) {
      const itemRet = (item as { Ret?: number } | undefined)?.Ret;
      if (itemRet !== undefined && itemRet !== 0) return false;
    }
