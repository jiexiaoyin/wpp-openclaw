...
function buildSessionKeyForMsg(msg) {
    ...
    let agentId = "main"; // 兜底 fallback (正常不会走这里)
    if (!accountCtx) {
...
function resolveLlmModel(msg) {
    try {
        ...
        if (e instanceof Error && e.message.startsWith("[WPP v1.5.0 P2-fix]"))
            throw e;
 ...
 */
const _outboundDedup = new Map(); // key → last sent ts (ms)
...
async function dispatchOne(msg, ctx = {}) {
    ...
    if (msg.trigger === "heartflow") {
        heartflowNote =
            "（注意：本次是你主动参与群聊的，不是用户叫你。回复应自然随意，像普通群成员一样加入话题。不要提\"我是机器人\"或解释你的机制。但如果话题涉及具体数据、业务信息或需要查证的事实，应主动查询你已掌握的相关技能/工具获取真实数据后再回答，不要编造、猜测或推脱说\"没查到/没数据\"。）";
    }
